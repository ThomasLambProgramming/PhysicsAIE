...
{
	//check for is trigger
	if (m_isTrigger)
	{
		//this will let us check every object that is inside a trigger object
		//and call triggerenter on if they havent registered inside the trigger this frame
	...
	if (glm::length(m_velocity) < 0.50f)
	{
	...
	if (abs(m_angularVelocity) < 1.01f)
	{
		m_angularVelocity = 0.0f;
	}
...
void Rigidbody::ResolveCollision(Rigidbody* a_otherActor, glm::vec2 a_contact, glm::vec2* a_collisionNormal, float a_pen)
{
	...
	//Register that these two objects have operlapped this frame
	...
	if (cp_velocity1 > cp_velocity2)
	{
		...
		float elasticity = (a_otherActor->GetElasticity() + GetElasticity()) / 2; 
		...
		{
		/*	ApplyForce(-impact, a_contact - m_position); 
			a_otherActor->ApplyForce(impact, a_contact - a_otherActor->m_position);*/
